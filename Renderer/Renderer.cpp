...
static const uint32_t NumProbeVertices = 4;

// Light distribution on the scene
static const uint32_t TopLights   = 0            + 0.2 * lightAmount;
static const uint32_t MiddleLights = TopLights   + 0.3 * lightAmount;
static const uint32_t BottomLights = MiddleLights + 0.5 * lightAmount;
...
void Renderer::loadMetal()
{
    ...
    for(uint8_t i = 0; i < MaxFramesInFlight; i++)
    {
        ...
        m_lightPositions[i] = m_pDevice->newBuffer(sizeof(float4)*lightAmount, storageMode);
    }
    ...
    // Setup render state to apply directional light and shadow in final pass
    {
        ...
        // Probe lights
        MTL::Function* pProbeVertexFunction = pShaderLibrary->newFunction( AAPLSTR( "probe_vertex" ) );
        MTL::Function* pProbeFragmentFunction = pShaderLibrary->newFunction( AAPLSTR( "probe_fragment" ) );

        AAPL_ASSERT( pProbeVertexFunction, "Failed to load probe_vertex shader" );
        AAPL_ASSERT( pProbeFragmentFunction, "Failed to load probe_fragment shader" );

        MTL::RenderPipelineDescriptor* pRenderPipelineDescriptor = MTL::RenderPipelineDescriptor::alloc()->init();

        pRenderPipelineDescriptor->setVertexDescriptor( nullptr );
        pRenderPipelineDescriptor->setVertexFunction( pProbeVertexFunction );
        pRenderPipelineDescriptor->setFragmentFunction( pProbeFragmentFunction );
        pRenderPipelineDescriptor->colorAttachments()->object(RenderTargetLighting)->setPixelFormat( colorPixelFormat );
        ...
        m_pProbePipelineState = m_pDevice->newRenderPipelineState( pRenderPipelineDescriptor, &pError );
        
        pRenderPipelineDescriptor->release();
        pProbeVertexFunction->release();
        pProbeFragmentFunction->release();
...
void Renderer::loadScene()
{
    ...
    m_meshes = newMeshesFromBundlePath("Meshes/JapaneseTemple24.obj", m_pDevice, *m_pDefaultVertexDescriptor, &pError);

    AAPL_ASSERT_NULL_ERROR( pError, "Could not create meshes from model file" );

    // Generate data
    {
        m_pLightsData = m_pDevice->newBuffer( sizeof(PointLight) * lightAmount, MTL::ResourceStorageModeShared );
    ...
    {
        SimpleVertex probeVertices[NumProbeVertices];
        const float angle = 2*M_PI/(float)NumProbeVertices;
        for(int vtx = 0; vtx < NumProbeVertices; vtx++)
        {
            int point = (vtx%2) ? (vtx+1)/2 : -vtx/2;
            float2 position = {sin(point*angle), cos(point*angle)};
            probeVertices[vtx].position = position;
        }

        m_pProbe = m_pDevice->newBuffer(probeVertices, sizeof(probeVertices), MTL::ResourceStorageModeShared);
    }
    ...
    // Create an icosahedron mesh
    {
        ...
        const float icoshedronRadius = 1.0;
    ...
    {
        m_pProbeMap = newTextureFromCatalog( m_pDevice, "ProbeMap", MTL::StorageModePrivate, MTL::TextureUsageShaderRead );
    }
}
...
void Renderer::populateLights()
{
    ...
    m_originalLightPositions = new float4[ lightAmount ];

    float4 *light_position = m_originalLightPositions;

    srandom(0x41fb3e8);

    for(uint32_t lightId = 0; lightId < lightAmount; lightId++)
    {
        float distance = 0;
        float height = 0;
        float angle = 0;
        ...
        if(lightId < TopLights)
        {
            distance = random_float(80,130);
            height = random_float(400,650);
            angle = random_float(0, M_PI*2);
            speed = random_float(0.003,0.014);
            speed *= (random()%2)*2-1;
        }
        else if(lightId < MiddleLights)
        {
            distance = random_float(140,180);
            height = random_float(210,350);
            angle = random_float(0, M_PI*2);
            speed = random_float(0.006,0.027);
            speed *= (random()%2)*2-1;
        }
        else if(lightId < BottomLights)
        {
            distance = random_float(120,250);
            height = random_float(50,200);
            angle = random_float(0, M_PI*2);
        ...
        *light_position = (float4){ distance*sinf(angle),height,distance*cosf(angle),1};
        ...
        if( lightId < TopLights) {
            light_data->light_color = (float3){0,0,3};
        } else if ( lightId < MiddleLights) {
            light_data->light_color = (float3){0,3,0};
        } else {
            light_data->light_color = (float3){3,0,0};
        }
...
void Renderer::updateWorldState( bool isPaused )
{
    ...
    frameData->probe_specular_intensity = 32;
    
    float cameraRotationRadians = m_frameNumber * 0.0025f + M_PI;

    camera->rotation.y = cameraRotationRadians * 180/ M_PI;
    
    // Receives keyboard input and moves camera following these inputs
    ...
    float4x4 transform = camera->TransformMatrix();

    float4x4 view_matrix = matrix_look_at_left_hand(0,  18, -50,
                                                    0,   5,   0,
                                                    0 ,  1,   0);

    view_matrix = view_matrix * transform;

    frameData->view_matrix = view_matrix;

    float4x4 templeScaleMatrix = matrix4x4_scale(0.05, 0.05, 0.05);
    ...
    float sunY = -3 * sin(m_frameNumber * 0.005f);
    
    float4 sunModelPosition = {-0.25, sunY, 1.0, 0.0};
...
void Renderer::drawableSizeWillChange(const MTL::Size& size, MTL::StorageMode GBufferStorageMode)
{
    ...
    m_projection_matrix = camera->ProjectionMatrix();
...
{
    pRenderEncoder->setRenderPipelineState( m_pProbePipelineState );
    pRenderEncoder->setDepthStencilState( m_pDontWriteDepthStencilState );
    pRenderEncoder->setCullMode( MTL::CullModeBack );
    pRenderEncoder->setVertexBuffer( m_frameDataBuffers[m_frameDataBufferIndex], 0, BufferIndexFrameData );
    pRenderEncoder->setVertexBuffer( m_pProbe, 0, BufferIndexMeshPositions );
    pRenderEncoder->setVertexBuffer( m_pLightsData, 0, BufferIndexLightsData );
    pRenderEncoder->setVertexBuffer( m_lightPositions[m_frameDataBufferIndex], 0, BufferIndexLightsPosition );
    pRenderEncoder->setFragmentTexture( m_pProbeMap, TextureIndexAlpha );
    pRenderEncoder->drawPrimitives( MTL::PrimitiveTypeTriangleStrip, 0, NumProbeVertices, lightAmount );
}
