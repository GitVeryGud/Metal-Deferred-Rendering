...
//    
//    gTextures[0] = new Texture(device);
//    gTextures[1] = new Texture(device);
//    gTextures[2] = new Texture(device);
//    
//    gTextures[0]->importTexture("Models/StructureBaseColor.png");
//    gTextures[1]->importTexture("Models/StructureNormals.png");
//    gTextures[2]->importTexture("Models/StructureSpecular.png");
//}
//
...
//    if (tangents != nullptr)
//        delete tangents;
//    tangents = new simd::float3[totalVertexAmount];
//    
//    if (bitangents != nullptr)
//        delete bitangents;
//    bitangents = new simd::float3[totalVertexAmount];
//    
...
//    aiVector3D currentAiVertex, currentAiNormal, currentAiTangent, currentAiBitanget;
//
//    for (int i = 0; i < assimpMesh->mNumVertices; i++) {
//        currentAiVertex = assimpMesh->mVertices[i];
//        currentAiNormal = assimpMesh->mNormals[i];
//        currentAiTangent = assimpMesh->mTangents[i];
//        currentAiBitanget = assimpMesh->mBitangents[i];
//        
//        vertices[i + numVertices] = currentAiVertex.x;
...
//        
//        tangents[i + numVertices].x = currentAiTangent.x;
//        tangents[i + numVertices].y = currentAiTangent.y;
//        tangents[i + numVertices].z = currentAiTangent.z;
//        
//        bitangents[i + numVertices].x = currentAiBitanget.x;
//        bitangents[i + numVertices].y = currentAiBitanget.y;
//        bitangents[i + numVertices].z = currentAiBitanget.z;
//        
...
//    const size_t sizeOfTanBuffer = sizeOfVertexBuffer;
//    const size_t sizeOfBitanBuffer = sizeOfVertexBuffer;
...
//    tangentsBuffer = device->newBuffer(sizeOfTanBuffer, MTL::ResourceStorageModeManaged);
//    bitangentsBuffer = device->newBuffer(sizeOfBitanBuffer, MTL::ResourceStorageModeManaged);
...
//    memcpy(tangentsBuffer->contents(), tangents, sizeOfTanBuffer);
//    memcpy(bitangentsBuffer->contents(), bitangents, sizeOfBitanBuffer);
...
//    tangentsBuffer->didModifyRange(NS::Range::Make(0, tangentsBuffer->length()));
//    bitangentsBuffer->didModifyRange(NS::Range::Make(0, bitangentsBuffer->length()));
...
//    pEnc->setVertexBuffer(textureBuffer, 0, 1);
//    pEnc->setVertexBuffer(normalsBuffer, 0, 2);
//    pEnc->setVertexBuffer(tangentsBuffer, 0, 3);
//    pEnc->setVertexBuffer(bitangentsBuffer, 0, 4);
//    
...
//        pEnc->setVertexBytes(localUniforms, sizeof(Snow_Uniforms), 5);
//        
//        for (int i = 0; i < stack[stackPtr - 1]->meshCount; i++) {
//            Mesh* currentMesh = stack[stackPtr - 1]->meshes[i];
...
//}
//
//void Model::renderDeferred(MTL::RenderCommandEncoder* pEnc) {
//    MeshNode* currentMeshNode = nullptr;
//    MeshNode* stack[256];
//    for (int i = 0; i < 256; i++)
//        stack[i] = nullptr;
...
//    pEnc->setVertexBuffer(bitangentsBuffer, 0, 4);
//    
//    pEnc->setFragmentTexture( gTextures[0]->texture, AAPLTextureIndexBaseColor );
//
//    pEnc->setFragmentTexture( gTextures[1]->texture, AAPLTextureIndexNormal );
//
//    pEnc->setFragmentTexture( gTextures[2]->texture, AAPLTextureIndexSpecular );
//    
//    stack[0] = baseNode;
//    stackPtr++;
//    
...
//}
//
//void Model::DrawDeferred(MTL::RenderCommandEncoder* pEnc){
//    renderDeferred(pEnc);
//}
//
